...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        //Feedback handling
        try{
                pool.query(`SELECT * FROM ??. ${interaction.message.embeds[0].footer.text.toLowerCase()} WHERE voterID = "${interaction.user.id}";`,[interaction.guild.id], (newError, result, fields) => {
                ...
                if (result.length == 0){
                    ...
                    pool.query(`INSERT INTO ??.${interaction.message.embeds[0].footer.text.toLowerCase()} (voterID) VALUES ("${interaction.user.id}");`,[interaction.guild.id], (insertError, InsertResult) =>{
